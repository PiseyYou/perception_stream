...
测试 `Proposal` 的图片 ID/哈希/尺寸、提议 ID、归一化框范围、裁剪变换、正负点、来源版本和配置哈希；测试 `CandidateAnnotation` 的实例 ID、双置信度、RLE 工件哈希、完整 provenance、状态枚举、失败错误码，以及未知标签的拒绝行为；测试受控别名、允许重叠关系、互斥组和 `thing_or_stuff` 标签配置的加载与校验。
...
从现有 `perception_streaming/robot_monitor/prelabel_pipeline` 的 `labels.csv`、`labels_mapping.yaml` 和导出映射构造规范标签；新增独立标签配置覆盖受控别名、`thing_or_stuff`、允许重叠关系、最小面积、互斥组与高风险标记。模型必须产出可序列化的版本、哈希和来源字段。
...
以临时图片文件夹和固定候选适配器夹具运行一次，断言生成不可变 `run_manifest.json`、CVAT XML、复核清单；再次执行同一输入和种子时结果字节一致；单图失败不阻断其他图片；全部图片失败时不可发布；取消和恢复只能复用哈希与配置一致的步骤；验证 GPU 批处理不超过配置上限、磁盘配额阻止写入并留下中文错误码、保留期清理只删除过期工件。
...
验证固定种子选择、10% 图片/每图 3 区域/每日预算限制、1536 px 裁剪、幂等键、20 秒超时和两次重试；断言普通中文日志不含原始请求、密钥或图片字节；验证请求必带供应商/模型/版本，密钥只从环境变量或本地密钥库读取；验证未知标签、越界坐标、无效 JSON、含提示注入指令的响应、直接发布多边形、绕过 DINO/SAM 定位的漏检结果和 API 失败均转 `failed` 或人工复核。

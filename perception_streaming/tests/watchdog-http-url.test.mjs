...
assert.match(
  ...
  'watchdog should allow the self-signed local certificate during health checks',
)
assert.match(
  source,
...
assert.doesNotMatch(
  source,
  /\n\s*sleep 8\n/,
  'watchdog should not use a fixed 8 second startup wait because HTTPS cold starts take much longer',
)

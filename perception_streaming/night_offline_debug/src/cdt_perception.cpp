...
//std::vector<std::string> class_names = {"cable_tie"};
...
cv::Mat cdt_perception::preprocessImage(const cv::Mat& img, float& y_scale, float& x_scale, int& y_shift, int& x_shift) {
    cv::Mat resize_img;

    if (preprocess_type == 1) { // LetterBox
        ...
        x_shift = (static_cast<int>(input_w) - new_w) / 2;
        int x_other = static_cast<int>(input_w) - new_w - x_shift;

        int new_h = static_cast<int>(img.rows * y_scale);
        y_shift = (static_cast<int>(input_h) - new_h) / 2;
        int y_other = static_cast<int>(input_h) - new_h - y_shift;
        ...
        cv::Size targetSize(static_cast<int>(input_w), static_cast<int>(input_h));
        cv::resize(img, resize_img, targetSize);
...
//    preprocessImage(mat, y_scale, x_scale, y_shift, x_shift);
...
std::vector<Detection> cdt_perception::postProcess(hbDNNTensor* out_tensor, float y_scale_ratio, float x_scale_ratio, int y_shift_len, int x_shift_len){
    // 初始化存储检测结果的容器
    std::vector<std::vector<Bbox>> bboxes(classes_num);
    std::vector<std::vector<float>> scores(classes_num);
    ...
    for (int cls_id = 0; cls_id < classes_num; cls_id++) {
        for (std::vector<int>::iterator it = indices[cls_id].begin(); it != indices[cls_id].end(); ++it) {
            float x1 = (bboxes[cls_id][*it].xmin - x_shift_len) / x_scale_ratio;
            float y1 = (bboxes[cls_id][*it].ymin - y_shift_len) / y_scale_ratio;
            float x2 = (bboxes[cls_id][*it].xmax - x_shift_len) / x_scale_ratio;
            float y2 = (bboxes[cls_id][*it].ymax - y_shift_len) / y_scale_ratio;

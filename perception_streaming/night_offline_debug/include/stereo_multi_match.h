...
//    ~StereoMultiMatch() = default;
    ...
    cv::Mat depth_;
    cv::Mat disparity_;
    // bool enable_height_filter_;  // 控制是否启用高度约束过滤

    void stereo_multi_param_init();
    cv::Mat create_label_mask(const cv::Mat& label_image, const std::vector<int>& labels);

    Mat stereo_multi_process_depth(Mat &rectifyL, Mat &rectifyR);
    // Mat stereo_multi_process_filter(Mat &disparity_, cv::Mat lab_dst);
    Mat stereo_multi_process_filter(Mat &disparity_, cv::Mat lab_dst, bool enable_height_filter_);

    // cv::Mat stereo_multi_process(cv::Mat& rectifyL, cv::Mat& rectifyR);
    ...
    // 新增:无过滤版本的点云生成函数
    void stereo_generate_pointcloud_no_filter(const Mat &depth, const Mat &lab, Mat &ori_mat,
                                              pcl::PointCloud<pcl::PointXYZRGBL> &xyz_rgbl_cloud,
                                              const std::string &save_path,
                                              Mat &three_view_output);

    // 新增：基于分割图补充缺失点云
    void stereo_supplement_missing_points(const Mat &depth, const Mat &lab, Mat &ori_mat,

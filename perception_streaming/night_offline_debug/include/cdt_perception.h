...
//
    ...
    std::vector<Detection> postProcess(hbDNNTensor* output, float y_scale, float x_scale, int y_shift, int x_shift);
    cv::Mat preprocessImage(const cv::Mat& img, float& y_scale, float& x_scale, int& y_shift, int& x_shift);
